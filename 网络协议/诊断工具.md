...
- `-i` 指定监听的网卡接口，例如 `eth0`、`ens18`，使用 `any` 可以抓取所有网卡；
- `-n` 不将主机名、端口号换为别名或域名，直接显示 IP 和数字端口，大幅提高查看速度；
- `-nn` 在 `-n` 的基础上，进一步不转换协议名和服务端口，例如显示 `80` 而非 `http`；
- `-c` 抓取指定数量的数据包后自动停止；
- `-w` 将抓包数据保存为 `.pcap` 格式的文件，方便导入 `Wireshark` 等图形界面工具中进行深度分析；
- `-r` 读取已保存的 `.pcap` 抓包文件；
- `-s` 设置每个数据包的抓取长度（单位为字节），默认是 `262144` 字节，使用 `0` 表示抓取完整的数据包；
- `-tttt` 以更加完整、易读取的格式，打印时间戳，例如 `2026-01-01 12:00:00.000000`；
- `-v` / `-vv` / `-vvv` 增加输出信息的详情程度，显示更多报文头部信息。

经典实战命令：

- `tcpdump -i eth0 -w dump.pcapng` 命令表示把经过 `eth0` 网卡上的数据包保存到 `dump.pcapng` 文件中；
- `tcpdump host 192.168.1.100` 命令表示抓取所有源地址或目的地址为 `192.168.1.100` 的数据包，`tcpdump src host 192.168.1.100` 命令表示抓取源地址为 `192.168.1.100` 的数据包，`tcpdump dst host 192.168.1.100` 命令表示抓取目的地址为 `192.168.1.100` 的数据包；
- `tcpdump port 80` 命令表示抓取通过 `80` 端口的所有数据包，`tcpdump dst port 443` 命令表示抓取目的端口为 443 的数据包；
- `tcpdump icmp` 命令表示抓取 ICMP 协议数据包，`tcpdump tcp` 表示抓取 TCP 协议数据包；
- `tcpdump src host 192.168.1.100 and port 80` 命令表示抓取源 IP 是 `192.168.1.100` 且端口是 `80` 的数据包，`tcpdump port not 22` 命令表示抓取端口不是 `22` 的数据包。
